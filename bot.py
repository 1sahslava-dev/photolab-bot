...
# Ссылка на Политику обработки персональных данных (заполни после публикации).
PRIVACY_POLICY_URL = os.environ.get(
    "PRIVACY_POLICY_URL", "https://telegra.ph/Politika-obrabotki-personalnyh-dannyh"
)
...
MODULE_NAMES = {
    1: "Модуль 1 — Истоки фотографии",
    2: "Модуль 2 — Что такое плёнка",
    3: "Модуль 3 — Плёночные камеры",
    4: "Модуль 4 — Объективы",
    5: "Модуль 5 — Экспозиция",
    6: "Модуль 6 — Фокус и резкость",
    7: "Модуль 7 — Свет и композиция",
    8: "Модуль 8 — Жанры и визуальный язык",
}


def modules_keyboard():
    buttons = []
    modules = sorted(set(t["module"] for t in COURSE))
    for m in modules:
        count = sum(1 for t in COURSE if t["module"] == m)
        name = MODULE_NAMES.get(m, f"Модуль {m}")
        buttons.append([InlineKeyboardButton(f"{name} ({count} тем)", callback_data=f"modsel_{m}")])
    buttons.append([InlineKeyboardButton("📊 Мой прогресс", callback_data="progress")])
    return InlineKeyboardMarkup(buttons)


def module_topics_keyboard(module, results=None):
    # Номер темы в кнопке — сквозной по всему курсу (Модуль 1: Т1–Т10,
    # Модуль 2: Т11–Т20 и т.д.), а не локальный номер внутри модуля из таблицы.
    buttons = []
    for global_num, t in enumerate(COURSE, start=1):
        if t["module"] != module:
            continue
        key = f"m{t['module']}_t{t['topic']}"
        mark = "✅ " if results and key in results else ""
        label = f"{mark}Т{global_num} — {t['title']}"
        buttons.append([InlineKeyboardButton(label, callback_data=f"start_{key}")])
    buttons.append([InlineKeyboardButton("◀ Все модули", callback_data="back_menu")])
    buttons.append([InlineKeyboardButton("📊 Мой прогресс", callback_data="progress")])
    return InlineKeyboardMarkup(buttons)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Бот сохраняет твои результаты тестов (Telegram ID и баллы), чтобы "
        f"показывать прогресс. Если ты пройдёшь контрольный тест на 100%, "
        f"для именного сертификата понадобится ввести имя и фамилию — это "
        f"отдельно и по желанию.\n"
        f"Подробнее: [Политика обработки персональных данных]({PRIVACY_POLICY_URL})\n\n"
        f"Выбери модуль:",
        parse_mode="Markdown", reply_markup=modules_keyboard(), disable_web_page_preview=True
    )
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    state = get_state(context)
    await ensure_results_loaded(update.effective_user.id, state)
    await update.message.reply_text("📚 Выбери модуль:", reply_markup=modules_keyboard())
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Результат темы ──
        ...
        await context.bot.send_message(
            ...
            parse_mode="Markdown", reply_markup=module_topics_keyboard(topic["module"], state["results"])
        )
    ...
    # ── Контрольный тест: результат ──
        ...
        if score == total:
            # 100% — прежде чем спросить имя и фамилию, явно спрашиваем согласие
            # на обработку этих данных (они нужны только для сертификата).
            ...
            text = (
                ...
                f"{grade}\n\n"
                f"🏆 *Ты набрал 100%!*\n"
                f"Для именного сертификата нужно ввести имя и фамилию — они "
                f"будут использованы только для оформления сертификата.\n"
            ...
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("✅ Согласен, ввести данные", callback_data=f"certconsent_{n}")
                ]])
            )
        else:
    ...
    # ── Согласие на обработку данных перед вводом имени для сертификата ──
    elif data.startswith("certconsent_"):
        n = int(data[len("certconsent_"):])
        state["awaiting_name"] = True
        state["cert_milestone_n"] = n
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text="Введи своё *имя и фамилию* — и я пришлю тебе именной сертификат:",
            parse_mode="Markdown"
        )
    ...
    # ── Назад в меню ──
        ...
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text="📚 Выбери модуль:", reply_markup=modules_keyboard()
        )

    # ── Выбор модуля ──
    elif data.startswith("modsel_"):
        module = int(data[7:])
        name = MODULE_NAMES.get(module, f"Модуль {module}")
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text=f"📚 {name}\nВыбери тему:",
            reply_markup=module_topics_keyboard(module, state.get("results"))
        )
